...
## 기능 목록
1. 입력
- [] 사용자로 부터 비상 근무 정보를 입력 받는다.
    - 월(숫자)와 시작 요일(일,월,화,수,목,금,토) 정보를 입력받는다.
    - 입력 형식에 옳바르지 않을 경우 `IllegalArgumentException`를 발생시킨다.

- [] 평일 근무자 정보를 입력 받는다.
    - 근무자는 최소 5명 에서 최대 35명까지 입력 받을 수 있다.
    - 입력된 근무자의 길이는 5글자를 초과할 수 없다.
    - 입력된 근무자의 닉네임은 중복될 수 없다.

- [] 주말 근무자 정보를 입력 받는다.
    - 근무자는 최소 5명에서 최대 35명까지 입력 받는다.
    - 입력된 근무자의 길이는 5글자를 초과할 수 없다.
    - 입력된 근무자의 닉네임은 중복될 수 없다.
